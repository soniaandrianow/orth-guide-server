(function() {
    ...
    function jhiAlertErrorController ($scope, AlertService, $rootScope) {
        ...
        var cleanHttpErrorListener = $rootScope.$on('orthGuideApp.httpError', function (event, httpResponse) {
            var i;
            ...
            case 400:
                var errorHeader = httpResponse.headers('X-orthGuideApp-error');
                var entityKey = httpResponse.headers('X-orthGuideApp-params');
                if (errorHeader) {
